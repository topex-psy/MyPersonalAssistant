...
function setBalloon(message, options = {}) {
  ...
  if (replies) {
    ...
    console.log('listActionEl', listActionEl);
...
function setAction(action, options = {}, callback = function(){}) {
  ...
  console.log("setAction", action, options);
...
function doChangeFacing() {
  ...
  div.classList.remove(previousFacing);
  div.classList.add(newFacing);
}
...
function doRandomWalk() {
  ...
  console.log('do random walk', delay, duration);
...
function doNothing(callback = function(){}) {
  ...
  myAssistant.el.setAttribute("data-activity", "");
  myAssistant.el.setAttribute("data-facing", "");
...
function setScale(scale = 1.0) {
  div.firstElementChild.style.bottom = (100 * scale) + 'px';
